...
class PrototypeDirichletEDLHead(nn.Module):
    """Prototype EDL head using Dirichlet evidence, not DST masses."""
    ...
    def __init__(
        ...
        super().__init__()
        if num_classes != 2:
            raise ValueError("PrototypeDirichletEDLHead currently expects binary output.")
...
class BagEmbeddingPrototypeEDLModel(nn.Module):
    def __init__(
        self,
        in_features,
        edl_dropout=0.0,
        ...
        self.edl_head = PrototypeDirichletEDLHead(
...
class MIL_PrototypeDirichletEDL_Wrapper(MIL_EDL_Wrapper):
    def __init__(
        self,
        mil_model,
        edl_dropout=0.0,
    ...
    def _make_proto_head(self, in_features, edl_dropout):
        return PrototypeDirichletEDLHead(
            in_features=in_features,
            num_classes=2,
    ...
    def prototype_heads(self):
        heads = {}
        if hasattr(self, "edl_head") and isinstance(self.edl_head, PrototypeDirichletEDLHead):
            heads["edl_head"] = self.edl_head
        for name, head in self.edl_side_heads.items():
            if isinstance(head, PrototypeDirichletEDLHead):
